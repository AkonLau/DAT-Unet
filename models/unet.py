...
from sub_models._radiounet import _RadioUNet
from sub_models._swimUnet import SwinTransformerSys
from sub_models._transUnet import TransUNet
...
class UNet(nn.Module):
    def __init__(self,
        ...
        self.model1 = RandomSample(img_size=img_size, patch_size=patch_size, in_chans=sample_chans)
        ...
        if model_type == 'DAT_UNet':
            ...
            self.model2 = _UNet(in_channels=in_channels, latent_channels=latent_channels, out_channels=out_channels,
                                features=features)
        elif model_type == 'CBAMUNet':
            self.model2 = _CBAM(in_channels=in_channels, latent_channels=latent_channels, out_channels=out_channels,
                                features=features)
        elif model_type == 'RadioUNet':
            self.model2 = _RadioUNet(in_channels=in_channels, latent_channels=latent_channels,
                                     out_channels=out_channels,
                                     features=features)

        elif model_type == 'TransUNet':
            self.model2 = TransUNet(img_size=img_size,
                                    in_chans=in_channels,
                                    out_chans=1,
                                    mid_embed_dim=96,
                                    head_num=4,
                                    ...
                                    block_num=8,
                                    patch_dim=16)
        elif model_type == 'SwimUNet':
            self.model2 = SwinTransformerSys(
                img_size=img_size,
                patch_size=4,
                in_chans=in_channels,
                out_chans=1,
    ...
    def forward(self, x, building_mask, image_cars, min_samples, max_samples, pre_sampled=False):
    ...
    def step(self, args, batch, optimizer, min_samples, max_samples, train=True, free_space_only=False):
        with torch.set_grad_enabled(train):
            if len(batch) == 4:
                ...
                complete_map, building_mask, image_cars = batch
            elif len(batch) == 2:
            ...
            map1, _, pred_map = self.forward(complete_map, building_mask, image_cars, min_samples, max_samples)
    ...
    def fit_wandb(self, train_dl, val_dl, optimizer, scheduler, min_samples, max_samples, project_name, run_name=None,
        ...
        args = kwargs['args']
        if run_name is None:
            run_name = f"{min_samples}-{max_samples} samples, {self.config['model_type']}"
            ...
            if args.resume != None:
                # replace simulation name with resume name
                resume_run_name = run_name.replace(args.simulation, args.resume)
                checkpoint_path = os.path.join(args.out_path, args.model_name, resume_run_name,
                                                   f'{args.epochs} epochs state dict.pth')
                if args.resume == 'DPM':
                    checkpoint_path = checkpoint_path.replace(', DPM', '')
                print(checkpoint_path)
                self.load_state_dict(torch.load(checkpoint_path))
                run_name += f", resume-{args.resume}"
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_dl):
                ...
                if pre_sampled:
                    ...
                    sample_map, sample_mask, pred_map = self.forward(sampled_map, building_mask, image_cars,
                                                                     min_samples,
                                                                     max_samples, pre_sampled=True)

                    pred_map *= sample_mask
                    pred_map += sample_map
                    ...
                    sample_map, sample_mask, pred_map = self.forward(complete_map, building_mask, image_cars,
                                                                     min_samples,
                                                                     max_samples, pre_sampled=False)
                ...
                if 'args' in kwargs:
                    ...
                    if args.visualization and i < 10:
                        visual_results = self.visualize_maps(complete_map, building_mask, sample_map, sample_mask,
                                                             pred_map)
